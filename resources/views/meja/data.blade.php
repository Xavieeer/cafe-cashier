...
<table class="table table-compact table-stripped" id="myTable">
    ...
    <tbody>
      ...
      <tr>
          ...
          <td>
              <button type="button" class="btn btn-primary btn-edit" data-toggle="modal" data-target="#formMejaEdit" 
              data-mode="edit" data-id="{{ $p->id}}" data-nomor_meja="{{ $p->nomor_meja}}"
              data-kapasitas="{{$p->kapasitas}}" data-status="{{$p->status}}">
                  <i class="fas fa-edit"></i>
              </button>
              </a>
              <form method="post" action="meja/{{$p->id}}" style="display:inline">
                  @csrf
                  @method('DELETE')
                  <button type="button" class="btn delete-data btn-danger" data-nomor_meja="{{ $p->nomor_meja}}">
                      <i class="fas fa-trash"></i>
                  </button>
              </form>
          </td>
