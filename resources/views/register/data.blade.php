    ...
    <tbody>
        @foreach ($user as $p)
            <tr>
                ...
                <td>{{ $p->email }}</td>
                <td>
                <span class="password-placeholder">Password Terenkripsi</span>
                </td>


                <td>
                    <form method="post" action="register/{{ $p->id }}" style="display:inline">
                        @csrf
                        @method('DELETE')
                        <button type="button" class="btn delete-data btn-danger" data-name="{{ $p->name }}">
                            <i class="fas fa-trash"></i>
                        </button>
                    </form>
                </td>
